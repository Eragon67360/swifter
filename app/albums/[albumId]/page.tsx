...
const AlbumDetailsPage = () => {
    ...
    const [isPlayerVisible, setIsPlayerVisble] = useState<boolean>(false);
        ...
        <>
            {album ? (
                <>
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4 space-x-5 ">
                                        <ul>
                                            {album.tracks.items.slice(0, Math.ceil(album.tracks.items.length / 2)).map((track, index) => (
                                                <li key={track.id} className={`break-words rounded-full bg-[#A5C9A5] px-4 py-2 my-2 transition-all duration-200 ${track.preview_url !== "" ? "cursor-pointer hover:scale-105" : "cursor-default"} cursor-pointer hover:scale-105`} onClick={() => playTrack(track.name)}>
                                                    {index + 1}. {track.name}
                                                </li>
                                            ))}
                                        </ul>
                                        <ul>
                                            {album.tracks.items.slice(Math.ceil(album.tracks.items.length / 2)).map((track, index) => (
                                                <li key={track.id} className="break-words rounded-full bg-[#A5C9A5] px-4 py-2 my-2 cursor-pointer hover:scale-105 transition-all duration-200" onClick={() => playTrack(track.name)}>{Math.ceil(album.tracks.items.length / 2) + index + 1}. {track.name}</li>
                                            ))}
                                        </ul>
                                    </div>
                                </div>
                    ...
                    <div className={`fixed bottom-0 left-0 z-50 grid w-full grid-cols-1 px-8 bg-white border-t border-gray-200 md:grid-cols-3 dark:bg-gray-700 dark:border-gray-600 transition-all duration-300 ${isPlayerVisible ? 'h-24' : 'h-0'}`}>
                        <div className="items-center justify-center hidden me-auto md:flex">
                            <Image src={album.images[0].url} alt={album.name} className="rounded-sm h-16 me-2" width={64} height={64} />
                        ...
                        <div className="flex items-center w-full">
                            <div className="w-full">
                                <div className="flex items-center justify-center mx-auto mb-1">
                                    ...
                                    <button data-tooltip-target="tooltip-pause" type="button" className="inline-flex items-center justify-center p-2.5 mx-2 font-medium bg-[#A5C9A5] rounded-full hover:bg-[#A5C9A5] group focus:ring-4 focus:ring-blue-300 focus:outline-none dark:focus:ring-blue-800">
                                ...
                                <div className="flex items-center justify-between space-x-2 rtl:space-x-reverse">
                                    ...
                                    <div className="w-full bg-gray-200 rounded-full h-1.5 dark:bg-gray-800">
                                        <div className="bg-[#A5C9A5] h-1.5 rounded-full w-[65%]"></div>
                                    </div>
                        ...
                        <div className="items-center justify-center hidden ms-auto md:flex">
                            ...
                            <div id="tooltip-playlist" role="tooltip" className="absolute z-10 invisible inline-block px-3 py-2 text-sm font-medium text-white transition-opacity duration-300 bg-gray-900 rounded-lg shadow-sm opacity-0 tooltip dark:bg-gray-700">
                                View playlist
                                <div className="tooltip-arrow" data-popper-arrow></div>
                            </div>
                            ...
                            <div id="tooltip-captions" role="tooltip" className="absolute z-10 invisible inline-block px-3 py-2 text-sm font-medium text-white transition-opacity duration-300 bg-gray-900 rounded-lg shadow-sm opacity-0 tooltip dark:bg-gray-700">
                                Toggle captions
                                <div className="tooltip-arrow" data-popper-arrow></div>
                            </div>
                            ...
                            <div id="tooltip-expand" role="tooltip" className="absolute z-10 invisible inline-block px-3 py-2 text-sm font-medium text-white transition-opacity duration-300 bg-gray-900 rounded-lg shadow-sm opacity-0 tooltip dark:bg-gray-700">
                                Full screen
                                <div className="tooltip-arrow" data-popper-arrow></div>
                            </div>
